...
class DQN:
    ...
    def select_action(self, cond, text, choices):
        ...
        #print(torch.max(logits.squeeze(), dim=0))
        return logits.squeeze()
        #return torch.max(logits.squeeze(), dim=0)
    ...
    def update(self):
        ...
        #with torch.no_grad():
            #ground_truth = reward + self.gamma * torch.max(self.target_net(input_ids_, token_type_ids_, choice_mask_),dim=1)[0] * (1 - done.long()) # [batch_sz, action_dim]
        #print(cond_, text_, choices_)
        next_q = torch.tensor([self.select_next_action(c, t, ch) for (c,t,ch) in zip(cond_, text_, choices_)])
        #next_q = torch.max(self.target_net(input_ids_, token_type_ids_, choice_mask_), dim=1)[0]
        #print('*'*100)
        #print(next_q)
        #print(reward)
        #print(flatten_offset)
